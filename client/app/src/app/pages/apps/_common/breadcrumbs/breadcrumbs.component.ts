...
export class BreadcrumbsComponent implements OnInit {
  ...
  handleClickCrumb(link: string) {
    if (link) {
      this.router.navigate([this.layoutService.rootRouter + link]);
    }
  }
  clickHome() {
    this.router.navigate([this.layoutService.rootRouter=="tt05/"?this.layoutService.rootRouter+"dashboard":this.layoutService.rootRouter])
  }
  ngOnInit() {
  }
}
